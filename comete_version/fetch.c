...
void METHOD(entry, ExecuteEvent)(void *_this, void *arg) {
  ...
  printf("Thread fetch 0x%.8x has been created\n", (unsigned int)pthread_self());
	...
	pthread_mutex_t mutex[NB_IDCT];
   	pthread_cond_t cond[NB_IDCT];
   	int id;
	for(id=0;id<NB_IDCT;id++){
...
char* obsbuffer=(char*)malloc(128*sizeof(char*));
//unsigned long long t=getCurrentTime();
//sprintf(obsbuffer,"%llu", t);
CUSTOM_SEND("begin time", "");
			...
			switch (marker[1]) {
				case M_SOF0: {
					...
					if (dispatch_info) {
						...
						CALL(REQUIRED.fetchDispatch, init,(void*)&SOF_section, YV, YH, flit_size);
						
						/*
						
						interface *itfDIspatch = retreiveInterface (comp, "fetchClientItf1", "client");
						message *msgDispatch;
						msgDispatch = createMessage (3000, 
										...
										(char *)& SOF_section,
										"R");
						send (msgDispatch, itfDIspatch->mbox);
...
// 						channelWrite (c[0], (unsigned char *) & flit_size, sizeof (uint32_t));
			...
			/* Initialisation de la fonction en face, qui va connaitre la bonne taille des flits */
						...
						//{
							CALL(REQUIRED.fetchCompute0, idct_init, flit_size, (void*)&(cond[0]), (void*)&(mutex[0]));
							CALL(REQUIRED.fetchCompute1, idct_init, flit_size, (void*)&(cond[1]), (void*)&(mutex[1]));
						//}
        ...
        // Start of Scan part
            ...
            // here to send the data to compute to the threads
						//message *msgBuffer;
						char *buffer = (char *) FLIT[to_idct_index];
						//channelWrite (c[to_idct_index + 1], (unsigned char *) FLIT[to_idct_index], flit_size * 64 * sizeof (int32_t));
							switch(to_idct_index){
								case 0:
									pthread_mutex_lock(&(mutex[0]));
									CALL(REQUIRED.fetchCompute0, idct_process,FLIT[to_idct_index]);
								...
								case 1:	
									...
									CALL(REQUIRED.fetchCompute1, idct_process,FLIT[to_idct_index]);
									VPRINTF("blocking on condition %p \n", &(cond[1]));
						...
						//CALL(REQUIRED.FETCH(fetchCompute,to_idct_index), idct_process,FLIT[to_idct_index]);
						
						/*msgBuffer = createMessage(1000+comp->componentID, comp->componentID, 1, "setBuffer", buffer, "R");
						interface *itf = retrieveInterface (comp, "fetchClientItf0", "client");
						send (msgBuffer, itf->mbox);*/
	...
	printf("end of file reading, ending computation\n");
	
CALL(REQUIRED.fetchCompute0, eof);
CALL(REQUIRED.fetchCompute1, eof);
