...
int fetch_process(Channel *c[NB_IDCT + 1])
{
   ...
   uint8_t index, index_X, index_Y, index_SOF;
   ...
   if ((movie = fopen(movie_name, "r")) == NULL) {
   ...
   while (!end_of_file) {
      ...
      if (marker[0] == M_SMS) {
         ...
         case M_SOF0:
            {
               ...
               /*\
                * Find the maximum dimension of vertical and
                * horizontal samples
               \*/
               for (index = 0; index < SOF_section.n; index++)
                  if ((SOF_component[index].HV & 0x0f) > max_ss_v)
                     max_ss_v = (SOF_component[index].HV & 0x0f);
               ...
               if (dispatch == 1) {
                  dispatch = 0;
                  nb_MCU_X = intceil(SOF_section.height, MCU_sx * max_ss_h);
                  nb_MCU_Y = intceil(SOF_section.width, MCU_sy * max_ss_v);
                  channelWrite(c[0], (unsigned char *)&SOF_section.width, sizeof(uint16_t));
                  channelWrite(c[0], (unsigned char *)&SOF_section.height, sizeof(uint16_t));
                  channelWrite(c[0], (unsigned char *)&nb_MCU_X, sizeof(uint16_t));
                  channelWrite(c[0], (unsigned char *)&nb_MCU_Y, sizeof(uint16_t));
                  channelWrite(c[0], (unsigned char *)&max_ss_h, sizeof(uint16_t));
                  channelWrite(c[0], (unsigned char *)&max_ss_v, sizeof(uint16_t));
                  channelWrite(c[0], (unsigned char *)&SOF_section.n, sizeof(uint16_t));
               }
         ...
         case M_DHT:
            {
               ...
               while (DHT_size < DHT_section.length) {
                  ...
                  tables[HT_type][HT_index] =
                      (huff_table_t *) malloc(sizeof(huff_table_t));
                  loaded_size =
                      load_huffman_table(movie, tables[HT_type][HT_index]);
                  if (loaded_size < 0) {
         ...
         case M_SOS:
            {
               ...
               for (index = 0; index < SOS_section.n; index++) {
                  ...
                  for (index_SOF = 0; index_SOF < SOF_section.n; index_SOF++) {
                     if (SOF_component[index_SOF].index ==
                         SOS_component[index].index) {
               ...
               /* Sends the component sizes in the proper order */
               for (index = 0; index < SOS_section.n; index++) {
                  uint32_t component_index = component_order[index];
                  channelWrite(c[0],
                               (unsigned char *)&SOF_component[component_index].HV,
                               sizeof(uint8_t));
               }
               ...
               for (index_X = 0; index_X < nb_MCU_X; index_X++) {
                  for (index_Y = 0; index_Y < nb_MCU_Y; index_Y++) {
                     for (index = 0; index < SOS_section.n; index++) {
                        ...
                        for (uint8_t chroma_ss = 0; chroma_ss < nb_MCU; chroma_ss++) {
                           unpack_block(movie, &scan_desc, index, MCU);
                           iqzz_block(MCU, unZZ_MCU,
                                      DQT_table[SOF_component[component_index].q_table]);
                           channelWrite(c[idct_index+1], unZZ_MCU, sizeof(unZZ_MCU));
                        }
               ...
               while (DQT_size < DQT_section.length) {
                  ...
                  IPRINTF
                      ("Quantization precision is %s\r\n",
                       ((DQT_section.pre_quant >> 4) & 0x0f) ? "16 bits" :
                       "8 bits");
         ...
         VPRINTF
             ("Invalid marker, expected SMS (0xff), got 0x%lx (second byte is 0x%lx) \n",
              marker[0], marker[1]);
      }
   }
