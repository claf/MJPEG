...
int main (int argc, char** argv)
{
  ...
  mjpeg_time_table = (time_mjpeg_t*) malloc (sizeof (time_mjpeg_t));
    mjpeg_time_table[0].tunpack = 0;
    mjpeg_time_table[0].twait = 0;
    mjpeg_time_table[0].tread = 0;
    mjpeg_time_table[0].tdec = 0;
    mjpeg_time_table[0].tcopy = 0;
    mjpeg_time_table[0].trsz = 0;
#endif
  ...
  sprintf (filename, "Trace_%s_seq%df_%ds_%db", argv[args - 1], framerate, sleep_time, frame_lookahead);
  gtg_init (filename);
...
noskip:
  ...
  GET_TICK (t1_1);
#endif
  decode_arg_t* decode_args = (decode_arg_t*) malloc (sizeof (decode_arg_t));
  decode_args->chunk =  chunk;
  decode_body (decode_args);
#ifdef C2X_USES_TIMING
  GET_TICK (t2_1);
  wq_time_table[tid].tpush += TICK_RAW_DIFF(t1_1,t2_1);
#endif
#ifdef MJPEG_USES_TIMING
                GET_TICK(t1);
...
clean_end:
  ...
  printf ("Time for thread %d :\t copy   :%ld\n",1, (long)tick2usec(mjpeg_time_table[0].tcopy));
  printf ("--------------------------------\n");
...
{
  fetch_arg_t* arg0 = (fetch_arg_t*) taskarg;
